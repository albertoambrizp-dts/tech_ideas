# -*- coding: utf-8 -*-
"""reporte_analisis.ipynb.py

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xILXJ_OTS-7MYvF8P8DJFEMvWN9iCArv
"""
...
try:
    scope = ['https://docs.google.com/spreadsheets/d/1Pq_qWcIACNw3A5j1Ptjopez3TYkWtLevEd69tSoLIh8/edit?gid=1648871531#gid=1648871531', 'https://www.googleapis.com/auth/drive']
    creds = ServiceAccountCredentials.from_json_keyfile_name(GOOGLE_SHEETS_JSON_PATH, scope)
    client = gspread.authorize(creds)

    sheet = client.open_by_key(APP_ID).worksheet('Respuestas')
    data = sheet.get_all_records()
    df_respuestas = pd.DataFrame(data)

    if len(df_respuestas) == 0:
        raise ValueError("No se encontraron respuestas en Google Sheets.")
    print(f"Se cargaron {len(df_respuestas)} respuestas.")

except Exception as e:
    print(f"¡ERROR CRÍTICO AL CARGAR GOOGLE SHEETS! {e}")
    print("Por favor, verifica: 1) APP_ID en .env, 2) 'credenciales_gsheets.json' existe, 3) la hoja 'Respuestas' existe.")
    # Crear un DataFrame vacío para no detener el notebook
...
if not df_respuestas.empty:
    model = SentenceTransformer('sentence-transformers/paraphrase-multilingual-MiniLM-L12')
    roles = df_respuestas['rol'].unique()
    rol_embeddings = {}

    # 6.1 Generar un embedding promedio por ROL
    for rol in roles:
        respuestas_rol = " ".join(df_respuestas[df_respuestas['rol'] == rol]['respuesta'].tolist())
        if respuestas_rol:
    ...
    pairs = [('Director', 'Analista'), ('Gerente', 'Analista')]
